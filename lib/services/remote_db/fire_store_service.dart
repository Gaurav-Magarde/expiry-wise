...
//   }
 ...
 /// -------------------------------------[Space Firebase] ----------------------------------
  ...
  //       await instance
  //           .collection('users')
  //           .doc(userId).update({'updated_at': DateTime.now().toString(),'members':FieldValue.arrayUnion([spaceId], )});
  //
  //     }
  //
  ...
  //        await docRef.update({'spaces' : spaces,'updated_at':DateTime.now().toString()});
  //       }
  //     }
  //   } on FirebaseAuthException catch (e){
  //     throw TFirebaseAuthException(e.code);
  ...
  //   batch.update(userRef, { 'spaces': FieldValue.arrayRemove([spaceId])});
  // }
...
//   }
  ...
  //   await FirebaseFirestore.instance.collection('spaces').doc(item.spaceId).collection('quick_list').doc(item.id).set(item.toMap(),SetOptions(merge: true));
  // }
  // Future<void> deleteFromQuickList({required String spaceId,required String id,}) async{
  //   await FirebaseFirestore.instance.collection('spaces').doc(spaceId).collection('quick_list').doc(id).delete();
  // }
