...
class LocalFirebaseSyncing {
  ...
  Future<void> performAutoSync() async{
    try{
      ...
      if(currentUser==null || currentUser.id.isEmpty || currentUser.userType=='guest') return;
      final isInternet =  ref.read(isInternetConnectedProvider);
      if(!isInternet) {
        SnackBarService.showToast('Auto sync failed.please check internet connection');
        return;
      }
  ...
  Future<void> performManualSync() async{
    try{
      ...
      final isInternet =  ref.read(isInternetConnectedProvider);
      if(!isInternet) {
        SnackBarService.showMessage('sync failed.please check internet connection');
        return;
      }
      await syncAllItemsNotSync();
   ...
   final users = await _sqfLiteSetup.getUserNotSynced();
    for (var user in users) {
      try {
        ...
        await _fireStoreService.insertUserTOFirebase(user);
    ...
    await ref.read(apiImageProvider).startSmartSync();
...
class FirebaseStreams {
  ...
  FirebaseStreams(this._fireStore, this._sqfLiteSetup, this._userRepository, this._fireStoreService, this.ref);

  Future<void> syncAllData(String? usersId,SyncType type) async {
    print("🚀 Starting Manual Sync...");

    final currentUser = await ref.read(currentUserProvider.future);
    if (currentUser == null || currentUser.id.isEmpty || currentUser.userType == 'guest') {
  ...
  Future<void> _syncItems(List<String> listSpace) async {
    for (String spaceId in listSpace) {
      ...
      try {
        ...
        final listNew = snapshot.docs.map((item)=>item.data()['id']??'').toSet();
        for(final item in local){
          if(listNew.contains(item.id)) {
            print('not deleted => ${item.name}');
            continue;
          }
        ...
        for (var doc in snapshot.docs) {
          ...
          print('itemmm  comed   ==>>>  ${item.name}');
          final local = await _sqfLiteSetup.fetchItemById(item.id);

          if (local != null && local.updatedAt == item.updatedAt) {
            print('itemmm  continued   ==>>>  ${item.name}');

            continue;
          }


           await ref.read(itemControllerProvider).insertItemFromFirebase( item: item,prev: local);
  ...
  Future<void> _syncSpaces(List<String> listSpace) async {
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        final localSpace = await _sqfLiteSetup.findSpaceBySpaceId(spaceId: space.id);
        ...
        for (MemberModel mem in members) {
          ...
          await _sqfLiteSetup.markMemberAsSynced(newM.id);
        }
  ...
  Future<void> _syncUsers(String userId) async {
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        if(localUser!=null && localUser.updatedAt == user.updatedAt) continue;
