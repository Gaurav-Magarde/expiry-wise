...
/// ---------------------------------------------------[Items Section]-------------------------------------
  ...
  Future<void> deleteAllSpaceItem({required String spaceId }) async {
    try{
      final db = await getDatabase;
      await db.delete('items',whereArgs: [spaceId],where: 'space_id = ?');
     refreshItems(spaceId: spaceId,);
    }catch(e){
      throw " ";
    }
...
// ---------------------------------------------------[Space Section]---------------------------------------
  ...
  Future<SpaceModel?> fetchCurrentSpace({required String spaceId,userId}) async {
    final db = await getDatabase;
    final map = await db.query("spaces", where: "id = ?", whereArgs: [spaceId]);
...
// -------------------------------------------[Member Section]-----------------------------------------
  ...
  Future<SpaceCardModel> fetchMemberBySpace({required String spaceId}) async {
    ...
    final mem = await ob.fetchMemberFromLocal(spaceId: spaceId);
  ...
  Future<void> updateSpace({required Map<String, String> map, required String id}) async {
    try{
      final db = await getDatabase;
      db.update('spaces',whereArgs: [id],where: 'id = ?',map);
    }catch(e){
      throw " ";
    }
  }
